                                          ...
                                          const vector<Blob<Dtype> *> &top)
{
   ...
   {
      CHECK_GE(background_label_id_, 0) << "background_label_id should be "
                                           "within [0, num_classes) for "
                                           "Softmax.";
      CHECK_LT(background_label_id_, num_classes_)
         << "background_label_id should be within [0, num_classes) for "
            "Softmax.";
      LayerParameter layer_param;
                                       ...
                                       const vector<Blob<Dtype> *> &top)
{
   ...
   // 由于bottom: "mbox_loc"输出的结果为axis=1的数据，因此num_就是其含有的元素个数
   // TODO: bottom[0]每个元素是用来干什么的？
   num_        = bottom[0]->num();
   num_priors_ = bottom[2]->height() / 4;
                                            ...
                                            const vector<Blob<Dtype> *> &bottom)
{
   ...
   if (propagate_down[0])
   {
      ...
      if (num_matches_ >= 1)
      {
         ...
         caffe_scal(loc_pred_.count(), loss_weight,
      ...
      if (num_conf_ >= 1)
      {
         ...
         if (do_neg_mining_)
         {
            ...
            for (int i = 0; i < num_; ++i)
            {
               ...
               const map<int, vector<int>> &match_indices =
                  all_match_indices_[i];
