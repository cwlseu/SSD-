   ...
   else if (code_type == PriorBoxParameter_CodeType_CORNER_SIZE)
   {
      ...
      else
      {
         ...
         encode_bbox->set_xmin((bbox.xmin() - prior_bbox.xmin()) / prior_width /
                               prior_variance[0]);
         encode_bbox->set_ymin((bbox.ymin() - prior_bbox.ymin()) /
                               prior_height / prior_variance[1]);
         encode_bbox->set_xmax((bbox.xmax() - prior_bbox.xmax()) / prior_width /
                               prior_variance[2]);
         encode_bbox->set_ymax((bbox.ymax() - prior_bbox.ymax()) /
                               prior_height / prior_variance[3]);
      }
   }
   else
   {
               ...
               vector<int> *match_indices, vector<float> *match_overlaps)
{
   ...
   for (int i = 0; i < num_pred; ++i)
   {
      ...
      {
         float overlap =
            JaccardOverlap(pred_bboxes[i], gt_bboxes[gt_indices[j]]);
         if (overlap > 1e-6)
         {
   ...
   for (int i = 0; i < num; ++i)
   {
           ...
           it != match_indices.end(); ++it)
      {
         ...
         if (mining_type == MultiBoxLossParameter_MiningType_MAX_NEGATIVE)
         {
            ...
            num_sel =
               std::min(static_cast<int>(num_pos * neg_pos_ratio), num_sel);
         }
         else if (mining_type == MultiBoxLossParameter_MiningType_HARD_EXAMPLE)
         {
...
{
   // 设置预测多少个location， 多少个是由bottom[0]->num()决定的
   loc_preds->clear();
   if (share_location)
   {
   ...
   for (int i = 0; i < num; ++i)
   {
      ...
      for (int p = 0; p < num_preds_per_class; ++p)
      {
         ...
         // 每个框框要对所有的种类都预测一遍，这个是对速度很大的限制的一个地方
         for (int c = 0; c < num_loc_classes; ++c)
         {
            int label = share_location ? -1 : c;
   ...
   for (int i = 0; i < num; ++i)
   {
           ...
           it != all_match_indices[i].end(); ++it)
      {
         ...
         for (int j = 0; j < match_index.size(); ++j)
         {
            ...
            const NormalizedBBox &gt_bbox =
               all_gt_bboxes.find(i)->second[gt_idx];
            ...
            if (bp_inside)
            {
               ...
               // When a dimension of match_bbox is outside of image region,
               // use gt_encode to simulate zero gradient.
...
void EncodeConfPrediction(
   ...
   const map<int, vector<NormalizedBBox>> &all_gt_bboxes, Dtype *conf_pred_data,
   Dtype *conf_gt_data)
{
   // CHECK_EQ(num, all_match_indices.size());
   ...
   while (score_index_vec.size() != 0)
   {
      ...
      for (int k = 0; k < indices->size(); ++k)
      {
         if (keep)
         {
            const int kept_idx = (*indices)[k];
            float overlap =
               JaccardOverlap(bboxes + idx * 4, bboxes + kept_idx * 4);
            keep = overlap <= adaptive_threshold;
         }
         else
         {
